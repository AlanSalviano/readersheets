...
def create_payroll_summary_with_vars_pdf(payroll_data, custom_variables):
    ...
    pdf.set_font("Arial", 'B', 16)
    pdf.cell(page_width, 10, txt="RESUMO DO PAYROLL", ln=1, align='C')
    pdf.ln(5)

    # Tabela do Resumo do Payroll
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(page_width, 10, txt="Resumo por Técnico", ln=1)

    headers_payroll = ["Técnico", "Pets", "Atend.", "Produzido", "Comissão", "Pgto Base", "Fixo", "Variáveis", "Pgto Final", "Support Value"]
    col_widths_payroll = [30, 15, 20, 25, 25, 25, 25, 25, 25, 25]

    pdf.set_font("Arial", 'B', 8)
    for i, header in enumerate(headers_payroll):
        pdf.cell(col_widths_payroll[i], 10, txt=header, border=1, align='C')
    pdf.ln()

    for row in payroll_data:
        pdf.set_font("Arial", size=8)
        pdf.cell(col_widths_payroll[0], 10, txt=str(row['Técnico'])[:15], border=1)
        pdf.cell(col_widths_payroll[1], 10, txt=str(row['Total de Pets']), border=1, align='C')
        pdf.cell(col_widths_payroll[2], 10, txt=str(row['Total de Atendimentos']), border=1, align='C')
        pdf.cell(col_widths_payroll[3], 10, txt=format_currency(row['Valor Produzido']), border=1, align='R')
        pdf.cell(col_widths_payroll[4], 10, txt=str(row['Comissao (%)']) + "%", border=1, align='C')
        pdf.cell(col_widths_payroll[5], 10, txt=format_currency(row['Pagamento Base']), border=1, align='R')
        pdf.cell(col_widths_payroll[6], 10, txt=format_currency(row['Pagamento Fixo']), border=1, align='R')
        pdf.cell(col_widths_payroll[7], 10, txt=format_currency(row['Variáveis']), border=1, align='R')
        pdf.cell(col_widths_payroll[8], 10, txt=format_currency(row['Pagamento Final']), border=1, align='R')
        pdf.cell(col_widths_payroll[9], 10, txt=format_currency(row['Support Value']), border=1, align='R')
        pdf.ln()

    # Linha de totalização
    total_pets_sum = sum(row['Total de Pets'] for row in payroll_data)
    total_atendimentos_sum = sum(row['Total de Atendimentos'] for row in payroll_data)
    total_produzido_sum = sum(row['Valor Produzido'] for row in payroll_data)
    total_pagamento_base_sum = sum(row['Pagamento Base'] for row in payroll_data)
    total_variaveis_sum = sum(row['Variáveis'] for row in payroll_data)
    total_pagamento_final_sum = sum(row['Pagamento Final'] for row in payroll_data)
    total_support_value_sum = sum(row['Support Value'] for row in payroll_data)

    pdf.set_font("Arial", 'B', 8)
    pdf.cell(col_widths_payroll[0], 10, txt="Total", border=1, align='C')
    pdf.cell(col_widths_payroll[1], 10, txt=str(total_pets_sum), border=1, align='C')
    pdf.cell(col_widths_payroll[2], 10, txt=str(total_atendimentos_sum), border=1, align='C')
    pdf.cell(col_widths_payroll[3], 10, txt=format_currency(total_produzido_sum), border=1, align='R')
    pdf.cell(col_widths_payroll[4], 10, txt="", border=1)
    pdf.cell(col_widths_payroll[5], 10, txt=format_currency(total_pagamento_base_sum), border=1, align='R')
    pdf.cell(col_widths_payroll[6], 10, txt="", border=1)
    pdf.cell(col_widths_payroll[7], 10, txt=format_currency(total_variaveis_sum), border=1, align='R')
    pdf.cell(col_widths_payroll[8], 10, txt=format_currency(total_pagamento_final_sum), border=1, align='R')
    pdf.cell(col_widths_payroll[9], 10, txt=format_currency(total_support_value_sum), border=1, align='R')
    pdf.ln(15)

    # Tabela de Variáveis Adicionais
    if custom_variables:
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(page_width, 10, txt="Variáveis Adicionais", ln=1)

        headers_vars = ["Técnico", "Descrição", "Valor da Parcela", "Total de Parcelas", "Parcela Atual", "Dívida Total", "Dívida Paga"]
        col_widths_vars = [30, 45, 25, 25, 25, 25, 25]

        pdf.set_font("Arial", 'B', 8)
        for i, header in enumerate(headers_vars):
            pdf.cell(col_widths_vars[i], 10, txt=header, border=1, align='C')
        pdf.ln()

        for row in custom_variables:
            pdf.set_font("Arial", size=8)
            pdf.cell(col_widths_vars[0], 10, txt=str(row['tech']), border=1)
            pdf.cell(col_widths_vars[1], 10, txt=str(row['description']), border=1)
            pdf.cell(col_widths_vars[2], 10, txt=format_currency(row['valor_da_parcela']), border=1, align='R')
            pdf.cell(col_widths_vars[3], 10, txt=str(row['total_de_parcelas']), border=1, align='C')
            pdf.cell(col_widths_vars[4], 10, txt=str(row['parcela_atual']), border=1, align='C')
            total_debt = row['valor_da_parcela'] * row['total_de_parcelas']
            pdf.cell(col_widths_vars[5], 10, txt=format_currency(total_debt), border=1, align='R')
            paid_debt = row['valor_da_parcela'] * row['parcela_atual']
            pdf.cell(col_widths_vars[6], 10, txt=format_currency(paid_debt), border=1, align='R')
            pdf.ln()
